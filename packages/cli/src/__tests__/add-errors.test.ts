...
async function runAdd(name: string): Promise<{ stderr: string }> {
  ...
  try {
    program.parse(['node', 'hareru', 'add', name]);
  } catch {
...
describe('hareru add — registry error', () => {
  beforeEach(() => {
    vi.resetModules();
  });
  afterEach(() => {
  ...
  it('shows styled error and build hint when registry not found', async () => {
    ...
    const { stderr } = await runAdd('Button');
