...
The Hareru CLI auto-detects your framework and generates CSS imports:
...
> When using `hareru init --write --mode tailwind`, the CLI wraps Hareru imports in a managed block and keeps `@import "tailwindcss"` outside it. The import order above is equivalent — `@layer` must appear before `@import "tailwindcss"`.
...
| `@hareru/ui/styles/components/*.css` | Per-component CSS (51 files) |
