...
Hareru UI is a semantic design system built for agents. It provides 55 accessible React components, a registry-first architecture, and first-class AI agent integration via MCP server, CLI, and machine-readable contracts.
...
- **CLI** — `npx @hareru/cli init --write` to set up your project, `add` to generate component CSS imports, and `update` to change CSS mode.
...
| `@hareru/ui` | 55 React components with BEM CSS |
| `@hareru/registry` | Shared registry types, loaders, and CSS mode recommendation |
| `@hareru/cli` | [CLI](/docs/cli) for component management — init, update, list, info, add |
