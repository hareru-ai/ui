...
Initialize your project and generate CSS imports:

```bash
# Initialize (auto-detects framework and CSS mode, creates managed block + hareru.json)
...
The CLI auto-detects your CSS entry file, package manager, and framework. When using `init`, the CLI writes a **managed block** to your CSS file:
...
For Tailwind projects, `init` uses the `tailwind` mode automatically and writes:
...
@import 'tailwindcss';  /* you manage this line — keep it after the block */
```

> The `@import 'tailwindcss'` line must come **after** the managed block so the `@layer` declaration takes effect.
